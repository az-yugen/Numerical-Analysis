...
A= [2 8 -2;2 10 4;2 12 2]             % Coefficient Matrix A
...
% SCALING
for k=1:n
    S(k) = max(abs(Aug(k,1:n)));      % determine the largest entry in kth row. store in S
end
fprintf("SCALING: Largest entry in each row:"); S


for k=1:n-1                           % for each kth column from 1 to n-1.
    
    for a=k:n
        d(a) = abs(Aug(a,k)/S(a));    % normalize kth column by largest entry
    end
    fprintf("SCALING: scaled column"); d
    
    % PARTIAL PIVOTING:
    fprintf("\n\n\n     PARTIAL PIVOTING LEVEL %i of %i: \n", k, n-1);
    [big, p] = max(d(k:n));   % finding max entry at kth column. big = max entry, p = which row it is at
    ...
    % SCALING
    for l=k:n
        S(l) = max(abs(Aug(l,1:n)));  % determine the largest entry in kth row. store in S
    end
    fprintf("SCALING: Largest entry in each row:"); S
